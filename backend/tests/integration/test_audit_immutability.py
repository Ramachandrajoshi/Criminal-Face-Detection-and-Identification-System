...
class TestAuditImmutability:
    ...
    def test_audit_log_sql_has_no_delete_grant(self):
        """Verify all migration files do not contain DELETE/UPDATE on audit_log."""
        # Search through all migration files in the migrations directory
        ...
        for fname in os.listdir(migrations_dir):
            if fname.endswith(".py") and fname not in ("runner.py", "__init__.py"):
                with open(os.path.join(migrations_dir, fname)) as f:
                    content += f.read()

        content_upper = content.upper()

        # audit_log table must not have UPDATE or DELETE statements
        assert "UPDATE AUDIT_LOG" not in content_upper
        assert "DELETE FROM AUDIT_LOG" not in content_upper
        assert "DELETE AUDIT_LOG" not in content_upper
